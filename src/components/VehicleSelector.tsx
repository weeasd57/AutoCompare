...
export function VehicleSelector({
    ...
    const [sortOption, setSortOption] = useState<
        'relevance' | 'year_desc' | 'hp_desc' | 'price_asc'
    >('relevance');
    ...
    const handleSelect = (vehicle: NormalizedSpec) => {
        ...
        if (selectedVehicles.length >= settings.maxCompareVehicles) {
            toast.error(
                `You can only compare up to ${settings.maxCompareVehicles} vehicles at a time.`
            );
            return;
        }
            ...
            <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-200 bg-gray-50 dark:border-dark-700 dark:bg-dark-800">
                ...
                <div className="flex-1 flex items-center gap-2">
                    {isSearch ? (
                        ...
                        <div className="text-sm md:text-base font-medium text-gray-900 dark:text-white ml-1 uppercase tracking-wider truncate">
                            {selectedModel
                                ? `${selectedMake} ${selectedModel}`
                                : selectedMake ||
                                  (variant === 'input' ? 'Select Make' : 'Vehicle Selector')}
                        </div>
                    )}
                </div>
            ...
            {suggestions.length > 0 ? (
                suggestions.map((vehicle) => (
                    ...
                    >
                        ...
                        <div>
                            <div className="text-gray-900 dark:text-white font-bold uppercase">
            ...
            {browseVehicles.length > 0
                ? browseVehicles.map((vehicle) => (
                      ...
                      >
                          ...
                          <div>
                              <div className="font-medium uppercase text-gray-900 dark:text-white">
                                  {vehicle.make} {vehicle.model}
                              </div>
                ...
                : availableYears.map((year: number) => (
                      <button
                          ...
                          onClick={() =>
                              handleSelect({
                                  id: `${selectedMake}-${selectedModel}-${year}`
                                      .toLowerCase()
                                      .replace(/ /g, '-'),
                                  make: selectedMake!,
                                  model: selectedModel!,
                      ...
                      >
                          <div className="w-10 h-10 flex items-center justify-center rounded-lg bg-gray-100 dark:bg-dark-600 group-hover:bg-blue-100 dark:group-hover:bg-primary-500/20 group-hover:text-primary-400 transition-colors">
                              <span className="font-bold text-xs text-gray-600 dark:text-gray-300 group-hover:text-blue-700 dark:group-hover:text-primary-400">
                                  {year}
                              </span>
                          </div>
                          <div>
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="flex-1 overflow-y-auto custom-scrollbar p-3 md:p-4">
                                {mode === 'search' ? <SearchResults /> : <BrowseContent />}
                            </div>
                        </div>
                    </div>
                )}
            ...
            {!isOpen ? (
                ...
                >
                    ...
                    <div className="text-center">
                        <span className="block text-lg font-semibold text-gray-300 group-hover:text-white">
                            Add Vehicle
                        </span>
