...
export async function generateComparisonPDF(
    ...
    doc.text('AutoCompare', margin, 25);
    ...
    doc.setFont('helvetica', 'normal');
    doc.text(
        'Generated by AutoCompare | Data sourced from NHTSA VPIC API',
        pageWidth / 2,
        yPosition,
        { align: 'center' }
    );
...
export async function exportComparisonToPDF(
    ...
    const blob = await generateComparisonPDF(comparison, highlights);
...
export function generateShareText(
    ...
    let text = `🚗 Vehicle Comparison: ${title}\n\n`;
    text += `Key Insights:\n`;
